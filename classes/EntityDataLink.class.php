...
class EntityDataLink
{
	...
	**/
	public function linkData($init, $site_id, $entity_type, $resource_type)
	{
		$entities_linked = '';
		$terms_linked ='';
...
// 	{
// 	  // drupal_set_message($entity_type.' '.$resource_type);
// 	  $matches = true;
// 	  $entities_linked = '';
// 	  $terms_linked ='';
	  
// 	  // check if matching action resource
// 	  $query = new EntityFieldQuery();
// 	  $query->entityCondition('entity_type', 'node')
// 	  ->entityCondition('bundle', 'src_actionresource')
// 	  ->fieldCondition('field_src_entity_type', 'value', $entity_type, '=')
// 	  ->fieldCondition('field_src_resource_type', 'value', $resource_type, '=');
...
// 	    		case 'node':
// 					// loop through node link table to finding matching nids
// 					$matches = db_query('SELECT nid, title FROM node WHERE type = :type', array(':type' => $resource_type));
// 					if($matches->rowCount() > 0)
// 					{
// 					// for each match check if link data doesn't exist
// 					foreach($matches as $match)
// 					{
// 					  // drupal_set_message('<pre>'.print_r($match, true).'</pre>','services');
// 					  $record_exists = db_query('SELECT 1 FROM services_rest_client_node_link WHERE service_site_id = :service_site_id AND client_nid = :client_nid', array(':service_site_id' => $site_id, ':client_nid' => $match->nid));
// 					  if($record_exists->rowCount() > 0){
// 					  }else{
...
// 					  	// create link
// 					    $fields_arr = array('service_site_id' => $site_id, 'client_nid' => $match->nid, 'service_nid' => $match->nid);
// 					    $node_link = db_insert('services_rest_client_node_link')->fields($fields_arr)->execute();
// 					      if($node_link){
// 					        $entities_linked .= $match->title.', ';
// 					      }
// 					  }
// 					}
// 					}else{
